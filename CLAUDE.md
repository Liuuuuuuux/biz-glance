...
```powershell
pnpm --filter @bizglance/cli dev workflow . --no-serve
```

## Workflow 参数

- `--full`：忽略 `.bizglance/intermediate/codegraph-assisted-input.json` 缓存；提供 `--context` 时使用手动上下文，否则重新生成确定性上下文和合并 findings。
...
```powershell
pnpm --filter @bizglance/cli dev workflow . --language zh
```
...
```powershell
pnpm --filter @bizglance/cli dev workflow . --full --no-serve
```

使用手动 CodeGraph-assisted context 调试：
